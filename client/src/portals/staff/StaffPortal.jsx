...
function initials(name) {
  return (name || '').split(' ').filter(Boolean).slice(0, 2).map(w => w[0]).join('').toUpperCase() || 'S';
}
...
export default function StaffPortal() {
  ...
  const [page, setPage] = useState(() => {
    const saved = localStorage.getItem('kid_staff_page');
    return STAFF_PAGE_KEYS.includes(saved) ? saved : 'dashboard';
  });
  ...
  function go(key) {
    setPage(key);
    localStorage.setItem('kid_staff_page', key);
  ...
  const pageProps = { go, toast, openModal, role: 'staff' };
            ...
            <div style={{ position: 'relative' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', padding: '4px 6px', borderRadius: 10 }} id="profile-btn" onClick={toggleProfile}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.2 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: '#0F172A', fontFamily: "'Poppins',sans-serif" }}>{user?.name || 'Staff'}</span>
                  <span style={{ fontSize: 11, color: '#64748B' }}>Clinic Staff</span>
                </div>
