...
export const DEFAULT_TEMP_PASSWORD = 'Kid@2026';
  ...
  return (
    <Modal title="Add New User" onClose={closeModal} width={440}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
        <div><label className="form-label">First Name</label><input id="au-first" type="text" className="form-input" placeholder="e.g. Maria" onInput={onNameInput('au-first-note')} /><div id="au-first-note" style={{ display: 'none', fontSize: 11, color: '#DC2626', marginTop: 4 }}>{INVALID_NAME_MSG}</div></div>
        <div><label className="form-label">Last Name</label><input id="au-last" type="text" className="form-input" placeholder="e.g. Santos" onInput={onNameInput('au-last-note')} /><div id="au-last-note" style={{ display: 'none', fontSize: 11, color: '#DC2626', marginTop: 4 }}>{INVALID_NAME_MSG}</div></div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
        ...
        <div><label className="form-label">Phone Number</label><input id="au-phone" type="tel" className="form-input" defaultValue="+63" placeholder="+63 000 000 0000" maxLength={16} onInput={e => { e.target.value = formatPhoneDisplay(filterPhoneInput(e.target.value)); }} /></div>
      </div>
