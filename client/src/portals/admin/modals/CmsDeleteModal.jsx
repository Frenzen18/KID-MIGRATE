...
export default function CmsDeleteModal({ data, closeModal }) {
  ...
  return (
    <Modal title={'Delete ' + labelCap} onClose={closeModal} width={440}>
      <div style={{ textAlign: 'center', padding: '8px 0 20px' }}>
        <div style={{ width: 52, height: 52, borderRadius: 14, background: 'var(--color-danger-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px', fontSize: 22, color: 'var(--color-danger)' }}><i className="fa-solid fa-trash" /></div>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}><button className="btn-secondary" onClick={closeModal}>Cancel</button><button style={{ padding: '9px 20px', borderRadius: 9, border: 'none', background: 'var(--color-danger-strong)', fontSize: 13, fontWeight: 600, color: '#fff', cursor: 'pointer' }} onClick={confirm}>Delete Permanently</button></div>
      </div>
    </Modal>
  );
}
