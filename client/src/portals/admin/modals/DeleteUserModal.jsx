...
export default function DeleteUserModal({ data, closeModal }) {
  ...
  return (
    <Modal title={<><i className="fa-solid fa-triangle-exclamation" style={{ color: 'var(--color-danger-strong)', marginRight: 8 }} />Delete User</>} onClose={busy ? undefined : closeModal} width={440}>
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
        ...
        <button className="btn-primary" style={{ background: 'var(--color-danger-strong)', borderColor: 'var(--color-danger-strong)', opacity: busy ? .8 : 1, cursor: busy ? 'default' : 'pointer' }} disabled={busy} onClick={confirm}>
          <i className={'fa-solid ' + (busy ? 'fa-spinner fa-spin' : 'fa-trash')} style={{ marginRight: 6 }} />{busy ? 'Deleting…' : 'Delete User'}
        </button>
      </div>
    </Modal>
  );
